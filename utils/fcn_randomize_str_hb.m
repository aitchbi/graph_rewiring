function [B,B0,energymin, init_energy] = fcn_randomize_str_hb(A,varargin)
...
%               B0,         randomized network with only degree preserve
%
%
...
if ~exist('B0','var')
    ...
    B0 = B; % only degree preserve
else
    B = B0;
end

if exist('JustGetB0','var')
    if JustGetB0
        B           = [];
        energymin   = [];
        init_energy = [];
