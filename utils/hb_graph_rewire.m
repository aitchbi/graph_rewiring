function [Y, outs] = hb_graph_rewire(X,varargin)
% HB_GRAPH_REWIRE rewires input graph such that the input degree sequence
% is preserved. This function is a warpper of an extended version of the
% function:
%
% subfcn_randomize_graph.m, Richard Betzel, 2013.
%
% The extensions enables a stronger rewiring than that provided by
% subfcn_randomize_graph.m while preserving the input degree sequence. The
% function also enables faster rewiring when the input graph is non-sparse,
% or even complete. The stronger randomization enables to better destroy
% the connectivity structure of the input graph, and, thus, may serve as a
% better degree-preserving null model that does not retain much of the
% input connectivity structure.
% 
% Inputs:
%   X: adjacency matrix of a non-directed graph with no self loops. Edges
%   may be binary or weighted.
%
% Outputs:
%   Y: rewired graph adjacency matrix; rewiring done by either adding new
%   edges, i.e., wiring nodes that were not wired in X, or swaping the
%   weights of existing edges. The desired number of rewires+weight-swaps
%   can be specified by optional variable "MaxSwap", otherwise the number
%   of rewires+weight-swaps is set to half the number of edges in the input
%   graph.
%
...
if isempty(opts.MaxSwap)
    opts.MaxSwap = floor(0.5*0.5*nnz(X));
end

N = size(X,1);

assert(issymmetric(X), 'input is not symmetric');
assert(isequal(diag(X), zeros(N,1)), 'input has self loops');

[Y, Y_orw,d] = subfcn_randomize_graph_hb(X, opts.MaxSwap, opts.DebugMode);

outs = struct;
outs.Y_only_rewire          = Y_orw; % only rewire (no weight-swap)
...
assert(issymmetric(Y), 'fishy: output is not symmetric');
assert(isequal(diag(Y), zeros(N,1)), 'fishy: output has self loops');
end

%==========================================================================
function [A, A_orw, out] = subfcn_randomize_graph_hb(A,MaxSwap,DebugMode)
% ----
...
%   Outputs:      A,      randomized matrix (has edge/weight swaps)
%             A_orw,      randomized matrix (has only edge swaps)
%
...
A_orw = A; % new A with only rewired (new) edges added 

%-Step 2: swap weights of exiting edges.
%--------------------------------------------------------------------------
% For edges that could not be rewired (which are many when working with a
% not-sparse graph), their weights are swaped to ensure the connectivity
% structure of the input graph is destroyed, such that only the degree
% sequence preserved. Note that swapping weights can also be seen as
% rewiring just that no "new" edge is created but the existing ones are
% swapped in pairs.
...
if DebugMode
    ...
    D_out_only_rewire = sum(logical(A_orw),1);
